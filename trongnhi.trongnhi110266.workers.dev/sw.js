...
self.addEventListener('fetch', e => {
    ...
    e.respondWith(caches.open(VERSION).then(x => x.match(q, {
        ignoreSearch: true
    }).then(hit => hit || fetch(q).then(r => {
        if (r.ok) x.put(q, r.clone());
        return r;
    }))));
});